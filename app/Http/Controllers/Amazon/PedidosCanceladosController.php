...
class PedidosCanceladosController extends Controller
{
    public function index() {
        return view('amazon.pedidos-cancelados');
    }

    public function search()
    {
        $pedidos = DB::table('pedido')
            ->where('estado', '=', EstadosPedido::$cancelado)
            ->get();
    ...
    public function enviarAPospuestos(Request $request)
    {
    ...
    public function deleteMultiple(Request $request)
    {
        ...
        foreach ($pedidos as $pedidoId) {
            DB::table('pedido')->where('id', $pedidoId)->delete();
        }
    }
}
